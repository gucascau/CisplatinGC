#!/usr/bin/env python3
"""
scVI / scANVI Annotation — Vehicle + Cisplatin (non-GC pair)

Adapted from Annotation/Cisplatin_GC_Vehicle_scvi_annotation_MKAOnly.py,
restricted to the Vehicle/Cisplatin query produced by
NewVersion/03_VehicleCisplatin_MultiMethod_Annotation.R (Step 3 h5ad export).

Runs scVI/scANVI label transfer onto the query dataset using one reference
atlas:

  - Mouse Kidney Atlas (MKA): MouseKidneyATLAS_MKA_updated.h5ad

Pipeline:
  1. Load the MKA reference h5ad; harmonise gene names and cell-type labels
  2. Load query h5ad (VehicleCisplatin_Raw_Predict_MKA_Lake_Annotation.h5ad)
...
Prerequisites: Run NewVersion/03_VehicleCisplatin_MultiMethod_Annotation.R
               first (Step 3) to generate the query h5ad. After this script
               finishes, re-run that R script to complete the merge (Step 4).

Output CSV columns/format match what NewVersion/03_...R's read-back section
expects exactly: first column "X" = query cell barcode with a "-query"
suffix (from AnnData.concatenate()'s default index_unique="-"), plus
...
ann_dir = "/home/gdzepedaorozcolab/lab/xxw004/Projects/RawDZscRNAseq/Results/Integration/VehicleCisplatin/Annotation/"

mka_path   = os.path.join(ref_dir, "MouseKidneyATLAS_MKA_updated.h5ad")
query_path = os.path.join(ann_dir, "VehicleCisplatin_Raw_Predict_MKA_Lake_Annotation.h5ad")
out_dir    = ann_dir
...
# Sanity check: MKA labels populated
...
# ── scVI / scANVI annotation using MKA reference ─────────────────────────────
LABEL_KEY = "cell_type"
ref_name  = "MKA"
...
# Shared genes
shared = adata_mka.var_names.intersection(adata_query.var_names)
print(f"  Shared genes (MKA ∩ query): {len(shared)}")

ref_sub   = adata_mka[:, shared].copy()
...
# ── Save predictions ─────────────────────────────────────────────────────────
# NOTE: deliberately NOT joining extra columns (Raw_cell_type,
# Seaurat_Transfer_Predicted_MKA, etc.) from adata_query.obs here, unlike the
# reference script. Those columns already live in the R Seurat object that
# will read this CSV back in (03_VehicleCisplatin_MultiMethod_Annotation.R,
# Step 4) via AddMetaData(); re-joining them here would hand AddMetaData
# duplicate column names for columns that already exist, which Seurat
# flags/overwrites unpredictably. Keeping the CSV to just the two scanvi_*
# columns avoids that collision entirely.
preds = combined.obs.loc[
    query_mask,
    [f"scanvi_{ref_name}_label", f"scanvi_{ref_name}_confidence"],
].copy()

csv_path = os.path.join(out_dir, "VehicleCisplatin_scanvi_MKA_annotations.csv")
