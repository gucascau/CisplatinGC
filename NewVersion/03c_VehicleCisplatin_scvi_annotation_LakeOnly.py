...
Adapted from Annotation/Cisplatin_GC_Vehicle_scvi_annotation_LakeOnly.py,
...
# ── Save predictions ─────────────────────────────────────────────────────────
# NOTE: deliberately NOT joining extra columns from adata_query.obs here —
# see 03b_VehicleCisplatin_scvi_annotation_MKAOnly.py for why (avoids an
# AddMetaData column-name collision when the CSV is read back into R).
preds = combined.obs.loc[
    query_mask,
    [f"scanvi_{ref_name}_label", f"scanvi_{ref_name}_confidence"],
].copy()

csv_path = os.path.join(out_dir, "VehicleCisplatin_scanvi_Lake_annotations.csv")
