  ...
  scrna.list[[i]] <-
    subset(
      ...
      subset = nFeature_RNA > 150  &
        nCount_RNA >= 150 &
        nFeature_RNA < 10000 &
        nCount_RNA < 20000 & percent.mt < 50 & rDNA < 40
    )
...
pdf("UroBladderOrganoid_Feature.pdf", height = 4, width = 6)
VlnPlot(scrna, features = c("nFeature_RNA"), group.by = "DataSet",pt.size=0 )
dev.off()

pdf("UroBladderOrganoid_nCounts.pdf", height = 4, width = 6)
VlnPlot(scrna, features = c("nCount_RNA"), group.by = "DataSet",pt.size=0 )
dev.off()

pdf("Cisplatin_GC_Vehicle_mtPercent.pdf", height = 4, width = 6)
...
#saveRDS(scrna, file = "Cisplatin_GC_Vehicle_scrna_merged_withoutdoublets.rds")
...
ggsave(filename = "Cisplatin_GC_Vehicle_SampleQC_reports.pdf",plot = VQCReport, height = 12, width = 4 )

IntegratedDir<- paste0(Outdir, "IntegratedFourConditions/")
...
# the average expression of these markers in the Cisplatin_GC_Vehicle
...
KidneyMarkersDotPlot<- DotPlot(scrna, features = markers_mouse_expanded_withothers) + RotatedAxis()
ggsave("KidneyMarkersDotPlot_Cisplatin_GC_Vehicle_Reclustered.pdf", plot=KidneyMarkersDotPlot, height=4, width=20)
