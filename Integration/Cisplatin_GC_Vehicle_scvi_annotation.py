...
Pipeline:
  ...
  3. For each reference independently:
       a. Subset to shared genes with query
       b. Train scVI on the reference
       c. Merge reference + query; train scANVI (semi-supervised)
       d. Predict cell types on query cells
       e. Save CSV and diagnostic plots
  4. Merge both per-reference predictions and save a combined CSV
...
# ── Per-reference scVI / scANVI annotation ───────────────────────────────────
LABEL_KEY = "cell_type"
all_preds = {}
...
# ── Merge predictions from both references and save ──────────────────────────
